...
const EventsInner = () => {
    ...
    const convertDate = (e: string) => {
        ...
        const timeStampCon = `${(d.getDate() < 10 ? '0' : '') + d.getDate()}.${
                            ...
                            events.map((item: EventPayload, id) => {
                                return (
                                    <div
                                        ...
                                        className={`event${item.codeChar === 'EV_AUTHENTIFICATION_DENIED' ? ' error' : ''}`}
                                    >
                                        <div className="event__header">
                                            <div className="event__header-info">
                                                <p className="p--sm--bold">{item.deviceName}</p>

                                                <p className="p--sm--bold">{convertDate(item.date)}</p>
                                            </div>
                                            ...
                                            >
                                                {(item.directionStr === 'Вход' || item.codeChar === 'EV_INPUT_REGISTERED') && (
                                                    <>
                                                        <EnterEvent />
                                                ...
                                                {(item.directionStr === 'Выход' || item.codeChar === 'EV_OUTPUT_REGISTERED') && (
                                                    <>
                                                        <ExitEvent />
                                        ...
                                        <div className="event__body">
                                            <div className="event__body-photo">
                                                {item.photo && item.photo !== 'Не определено' ? (
                                            ...
                                            <div className="event__body-info">
                                                {item.codeChar !== 'EV_AUTHENTIFICATION_DENIED' ? (
                                                    <>
                                                        {item.personFirstName && item.personFirstName !== 'Не определено' ? (
                                                            <>
                                                                <div className="employee-surname">
                                                                    <div className="p--md--bold">{item.personLastName}</div>
                                                                </div>
                                                    ...
                                                    <div className="employee-error">
                                                        ...
                                                        {item.identificatorContent ? (
                                                            ...
                                                            <div className="p--sm--bold">Внимание! Лицо не опознано</div>
                                                        )}
                                                    </div>
                                                )}
                                            </div>
                                        ...
                                        <div className="event__footer">
                                            ...
                                            {(item.codeChar === 'EV_OUTPUT_REGISTERED' || item.codeChar === 'EV_INPUT_REGISTERED') && (
